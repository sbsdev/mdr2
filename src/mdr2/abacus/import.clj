...
(defn print-stats
  ...
  (println "---------")
  (println)
  (println "Recording with no wav and a struct.html:" (count (filter #(recording-no-wav-with-struct? %) all-commercial-productions)))
  (println "----------------------------------------")
  (println (string/join ", " (map :id (filter #(recording-no-wav-with-struct? %) all-commercial-productions))))
  (println)
  (let [weird (filter #(and (recording-no-wav? %)) all-commercial-productions)]
    (println "Recording with no wav:" (count weird))
    (println "----------------------")
    (println (string/join ", " (map :id weird))))
  (println)
  (println "Recording with wav and an obi project:" (count (filter recording-with-obi? all-commercial-productions)))
  (println "--------------------------------------")
  (println (string/join ", " (map :id (filter recording-with-obi? all-commercial-productions))))
  (println)
  (println "Recording with wav and no obi project:" (count (filter recording-with-wav-no-obi? all-commercial-productions)))
  (println "--------------------------------------")
  (println (string/join ", " (map :id (filter recording-with-wav-no-obi? all-commercial-productions))))
  (println)
  (print "Ignoring the rest: ")
  (println (dissoc (frequencies (map :state all-commercial-productions)) "archived" "recording" "ready"))
  (println "------------------")
  (doseq [state ["structuring" "wipsing"]]
    (println (str (string/capitalize state) ":") (string/join ", " (map :id (filter #(= (:state %) state) all-commercial-productions)))))
  (println )
)
